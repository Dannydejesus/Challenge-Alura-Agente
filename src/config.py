...
# Fix encoding UTF-8 en Windows (evita errores con caracteres especiales)
if sys.stdout.encoding != "utf-8":
    sys.stdout.reconfigure(encoding="utf-8")
if sys.stderr.encoding != "utf-8":
    sys.stderr.reconfigure(encoding="utf-8")
...
if not GOOGLE_API_KEY:
    raise ValueError(
        ...
        "Crea un archivo .env basado en .env.example y agrega tu API Key."
    )
...
CHROMA_DB_PATH = os.getenv("CHROMA_DB_PATH", str(CHROMA_DIR))
CHROMA_COLLECTION_NAME = os.getenv("CHROMA_COLLECTION_NAME", "rh_documents")
...
APP_TITLE = os.getenv("APP_TITLE", "Agente RH Corporativo")
APP_DESCRIPTION = os.getenv(
    "APP_DESCRIPTION",
    "Asistente de Recursos Humanos basado en documentos corporativos"
)
...
logger.info(f"✅ Configuración cargada | Modelo LLM: {LLM_MODEL} | Modelo Embedding: {EMBEDDING_MODEL}")
